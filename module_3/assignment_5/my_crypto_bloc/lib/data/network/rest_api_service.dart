...
import 'package:my_crypto_bloc/data/data.dart';
...
class RestApiService {
  ...
  Future<List<CryptoSummary>> getHomePageCrypto() async {
    ...
    if (response.statusCode == 200) {
      ...
      throw GetHomepageCryptoException(
          'API Error getting homepage coins summary');
    }
  }
  ...
  Future<List<CryptoSummary>> searchCryptoByKeyword(String keyword) async {
    ...
    if (response.statusCode == 200) {
      ...
      List<CryptoSummary> cryptos =
          raw.map((crypto) => CryptoSummary.fromSearchJson(crypto)).toList();
      return cryptos;
    } else {
      throw SearchCryptoException('API Error when searching coins');
    }
  }
  ...
  Future<CryptoDetails> getCryptoDetailsById(String cryptoId) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw GetCryptoDetailsException(
          'API Error getting crypto details by cryptoId: $cryptoId');
    }
  }
}
